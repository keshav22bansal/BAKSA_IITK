...
def RNN(X_train,y_train,args):
	"""
	Purpose -> Define and train the proposed GRU network
	Input   -> Data, Labels and model hyperparameters
	Output  -> Trained GRU network
	"""
	...
	# Input -> Embedding -> Conv1D+Maxpool1D -> GRU -> GRU -> FC-1 -> Softmaxloss
	print('Build model...')
	model = Sequential()
...
def save_model(Masterdir,filename,model):
	...
	model.save_weights(Masterdir+'Models/GRU_'+filename+'_weights.h5')
	json_string = model.to_json()
	f = open(Masterdir+'Models/'+'GRU_'+filename+'_architecture.json','w')
	f.write(json_string)
	f.close()
...
if __name__ == '__main__':
	...
	model = RNN(deepcopy(X_train),deepcopy(y_train),[MAX_FEATURES, MAXLEN, embedding_size,\
			     filter_length, nb_filter, pool_length, gru_output_size, batch_size, \
